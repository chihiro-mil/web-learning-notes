...
# コンフリクト発生時の対処手順
1. VSコードでコンフリクトの原因になっている箇所の差分表示する（色分けされているので分かりやすい）
2. 手動でどの変更を残すか修正
3. VSコード上に記載されている「<<<< HEAD」、「====」、「>>>> branch-name」などは必ず削除する
4. git add でステージングエリアに上げる（コンフリクト発生時はマージ完了していないため）
5. git commit でコンフリクト解決を確定する